...
public class BuyProcessingState extends TradeState {
	...
	public void onEnter() {
		...
		if (_context.getController().getPortNumber() == TradingConstants.REAL_MONEY_TRADING_PORT_NUMBER) {
			_context.setQuantity(StockUtils.calculateQuantity(MarketMonarch._preTradeContext.getTotalCashBufferedInUsd(), MarketMonarch._tradingContext.getEntryPrice()));	
		} else {
