...
public final class ReportPdfCreator {
	...
	public static void createSessionReport(TradeReportDto reportData, String filePath) throws IOException {
		...
		// Page 1 ==============================================================
		// Add content to page
		PDPageContentStream contentStream = new PDPageContentStream(document, pageWithStats);
		float textMargin = 50;
		...
		contentStream.setNonStrokingColor(new Color(100, 149, 237));
		...
		for (String[] row : rows) {
			contentStream.beginText();
			contentStream.newLineAtOffset(labelX, textY);
			contentStream.showText(row[0]);
			contentStream.endText();

			contentStream.beginText();
			contentStream.newLineAtOffset(valueX, textY);
		...
		contentStream = new PDPageContentStream(document, pageWithDiagram);
		// Size of the diagram
